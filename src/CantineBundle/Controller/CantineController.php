...
class CantineController extends Controller
{
    public function ajouterPlatAction(Request $req)
    {
        ...
        $eleve = $this->getDoctrine()->getRepository(User::class)->findAll();
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $transport = Swift_SmtpTransport::newInstance('smtp.googlemail.com',465, 'ssl')
                ->setUsername('elyes.tarmiz@esprit.tn')
                ->setPassword('elyes19081998');
...
foreach ($eleve as $e) {
    $message = (new \Swift_Message('Nouveau Plat :) ! '))
        ...
        ->setTo($e->getEmail())
        ->setBody('Un nouveau plat est à présent servi dans votre cantine 
        allez donnez votre avis ! ');
    $mailer->send($message);
    $this->get('mailer')->send($message);
}
           // return new Response('youp');
